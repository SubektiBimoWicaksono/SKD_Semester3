...
function hillCipherDecrypt(ciphertext, keyMatrix) {
    ...
    function numberToChar(number) {
      return String.fromCharCode((number + 26) % 26 + 'A'.charCodeAt(0));
    }
    ...
    for (let i = 0; i < ciphertext.length; i += blockSize) {
      ...
      const decryptedChars = decryptedBlock.map(numberToChar); // Corrected line
  ...
  // Chipertext
  const ciphertext = 'RLMKEIPVOCUMUQSKKKOWYGMSSOU';
  
  // Dekripsi chipertext menggunakan Hill Cipher
  const plaintext = hillCipherDecrypt(ciphertext, keyMatrix);
